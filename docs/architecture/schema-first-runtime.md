...
The production binary is built exclusively from goforms/cmd/api. That composition root performs explicit construction in this order:
    ...
    config -> logger -> PostgreSQL connection
           -> form/outbox repository + token repository
           -> v1 HTTP handler -> Echo router -> net/http server
           -> optional safe webhook dispatcher -> public HTTPS destination
...
| Webhook delivery | internal/application/webhook and internal/domain/webhook | narrow outbox interface, authenticated encryption, net/http |
...
When a form has an enabled webhook, that same transaction snapshots its encrypted headers, signing secret, and destination into a pending outbox record. The optional dispatcher claims records with FOR UPDATE SKIP LOCKED, revalidates public DNS at connection time, signs the immutable event, and records retry, delivered, or dead-letter state. Endpoint and delivery APIs remain owner-scoped and never expose encrypted or plaintext secret fields.
...
The former web application, renderer fork, dependency-injection modules, browser sessions, CSRF, assertion auth, plan enforcement, shadow users, and duplicate middleware have been deleted from the working tree. The pre-removal commit and archived repository links are recorded in `docs/archive/legacy-runtime.md`.

The cmd/api route test enforces the cutover by rejecting every `/api/` route and proving retired assertion headers cannot authenticate a v1 request. CI, dependency, and security scans run only against the supported Go runtime and its contracts.
...
- Reintroducing browser sessions, CSRF, assertion headers, shadow users, plan headers, or a renderer-specific schema into v1 requires a new ADR and explicit roadmap change.
